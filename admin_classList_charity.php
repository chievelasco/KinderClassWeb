...
if ($results) {
  while ($row = mysqli_fetch_assoc($results)) {
    ...
    if (isset($_POST["save_btn"])) {
      ...
      header("Location: admin_classList_charity.php");
      exit();
    }
...
<body>
  ...
  <section class="class_list-section">
    <?php $query = "SELECT * FROM admin_login WHERE admin_ID = 2";
    $result = mysqli_query($conn, $query) or die("Error in query: " . mysqli_error($conn));
    $row = mysqli_fetch_assoc($result) or die("No user found with the given ID.");

    $section2 = $row['section'];
    ?>
    <div class="text">Class List -  <?php echo $section2 ?> </div>
    <div class="grid auto-fit">

      <?php

      $query = "SELECT * FROM reg_info WHERE section = 'Charity' ";
      $results = mysqli_query($conn, $query);
      if ($results) {
        while ($row = mysqli_fetch_assoc($results)) {
          $firstName = $row['stud_fname'];
